...
const AdminNoticeDetailView = ({
  ...
  const [confirmType, setConfirmType] = useState<'delete' | 'popup' | null>(
    null,
  );
            ...
            {isAdmin ? (
              <div className="flex w-full justify-end gap-3">
                <button
                  ...
                  disabled={
                    !notice.id || isLoading || isDeleting || isSettingPopup
                  }
          ...
          <div className="mx-auto flex max-w-[980px] flex-col gap-14">
            ...
            <article className="flex flex-col gap-8 text-[#202124]">
              ...
              {!isLoading && !isError && notice.attachments.length ? (
                <section className="rounded-[12px] border border-[#E6ECFF] bg-[#F7F9FF] px-6 py-6 md:px-8">
                  <div className="flex-col gap-2">
                    <h2 className="caption2 text-gray-900">첨부파일</h2>

                    <div className="flex-col divide-y divide-[#DCE4FF]">
                      {notice.attachments.map((file) => (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="flex-row-between caption3 gap-4 transition-colors hover:text-[#3557F6]"
                        >
                          <span className="min-w-0 truncate text-gray-900">
                            {file.fileName}
                          </span>
                          <span className="caption3 shrink-0 text-[#5B74F7]">
                            다운로드
                          </span>
                        </a>
                      ))}
