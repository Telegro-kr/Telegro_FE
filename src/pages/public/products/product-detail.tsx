...
const ProductDetail = () => {
  ...
  if (!hasValidProductId) {
    return <MessagePanel message="유효하지 않은 상품입니다." />;
  }

  if (productDetailQuery.isLoading) {
    return <MessagePanel message="상품 정보를 불러오는 중입니다." />;
  }

  if (productDetailQuery.isError || !productDetailQuery.data?.data) {
    return <MessagePanel message="상품 정보를 불러오지 못했습니다." />;
  }
...
const MessagePanel = ({ message }: { message: string }) => (
